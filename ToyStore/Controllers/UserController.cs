...
namespace SourceCode.Controllers
{
    public class UserController : Controller
    {
        ...
        public ActionResult ConfirmEmail(int ID)
        {
            ...
            //SentMail("Mã xác minh tài khoản SourceCode", email, "huynhanhvu7749@gmail.com", "jrdkxkiekvcnkcbl", "<p>Mã xác minh của bạn: " + captcha + "<br/>Hoặc xác minh nhanh bằng cách click vào link: " + urlBase + "/User/ConfirmEmailLink/" + ID + "?Capcha=" + captcha + "</p>");
            //return View();
            User user = _userService.GetByID(ID);
            ViewBag.Message = "EmailConfirmed";
            //Gift
        ...
        public ActionResult ListCustomer(int page = 1)
        {
            if (Session["User"] == null)
            {
            ...
            //Get qAs list
            IEnumerable<User> qcus = _userService.GetList();
            PagedList<User> listcus = new PagedList<User>(qcus, page, pageSize);
            //Check null
            if (listcus != null)
            {
